...
if admission_type == "내신(수시) 기준":
    ...
    # 버튼을 누르지 않았을 때는 폭죽과 멘트를 보여주지 않음
    if st.button("✨ 내신 기반 추천 대학 확인하기"):
        if gpa_input: # 입력값이 있을 때만 결과 처리
            # 두구두구 멘트 추가
            st.markdown("## 🥁 두구두구...!")
            ...
            if recommendation['universities']:
                ...
                if recommendation['lifestyle_tips']:
                    ...
                    for tip in recommendation['lifestyle_tips']:
                        st.write(f"- {tip}")
            else: # 입력값이 있으나 추천 대학이 없는 경우 (예: 정보 없음 티어)
                st.warning(f"⚠️ {recommendation['note']}")
        else: # 입력값이 없을 때 (최초 로드 시나 미입력 시)
    ...
    if st.button("✨ 수능 기반 추천 대학 확인하기"):
        if suneung_input: # 입력값이 있을 때만 결과 처리
            # 두구두구 멘트 추가
            st.markdown("## 🥁 두구두구...!")
            st.balloons() # 폭죽 (풍선) 효과
            time.sleep(1) # 잠시 기다려 시각적 효과를 강조
